...
The Phase 2b candidate ledger, its per-stage evidence artifacts, the JSON
schemas that validate them, the preregistration contract, the evaluation
protocol, and the research work log are kept in a separate private repository
together with their 95 regression tests.

They are not published here because the ledger records screening verdicts about
public repositories belonging to third parties, including individual and
student projects. Those verdicts decide eligibility for a research protocol;
they are not quality judgments of those projects, and naming them alongside a
verdict is not something this repository publishes.

The separation costs this repository nothing it relies on. Nothing here claims
a routing result, an agent ranking, or a comparative measurement, so no public
artifact depends on the withheld chain. If a routing policy is ever promoted to
the default, the protocol and aggregate results will be published here as a
summary rather than as row-level verdicts.
