...
passes plus the previously passed `candidate-a #288` row.
...
tasks terminate on `translation-only`, and the broad `candidate-b #403`
...
the final candidate-c validity reviewer must decide whether the offline roster
...
candidate-d rollback structural assertion must become a behavioral failure-and-restore
...
feasibility passes 290; `a benchmark row` remains unknown because
