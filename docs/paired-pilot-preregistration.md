...
**이 실험의 산출물은 공개 연구물이나 배포용 dataset이 아니라, 이 오케스트레이터에 반영할
routing 방법론이다.** fixture와 agent 산출물은 그 방법론을 도출하는 중간 재료이며 배포
대상이 아니다. 이 전제가 아래 경계의 근거이고, 전제가 바뀌면 이 절을 먼저 개정한다.
...
판단을 뒤집는다. 그 논리는 **재배포하지 않는다는 전제에서만** 성립하고, 목표가 법적 여유
확보로 바뀌면 이 선별은 자의적 선택이 아니라 목표에 부합하는 요건이 된다.
