...
Phase 2a의 4-task smoke 전용이다. v2 4-task 계약 rehearsal은 agent-free preflight까지
통과했다. 다음 gate는 이 manifest를 독립 검토한 뒤 별도 승인이 있을 때만 실제 8회
재실행하는 것이며, 그 결과와 무관하게 60-task pilot용 일반 schema는 별도로 설계한다.
...
Phase 2a tooling은 완료됐으므로 실제 4-task manifest/evaluator 사전 등록부터 진행한다.
사용자 요청 없이
