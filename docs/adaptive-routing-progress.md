...
> 마지막 갱신: 2026-07-18
> 상태: 설계 검토와 Phase -1 추가 오염 차단 완료, Phase 0A evaluator 의미 분리 대기
...
- [ ] Phase 0A evaluator role/API 의미 분리
- [ ] Phase 0B legacy evaluator migration smoke
...
그 다음 이 문서의 “지금 작업 중”에서 첫 미완료 항목을 선택한다. Phase -1은
완료됐으므로 Phase 0A의 evaluator API 결정을 확정하고 Phase 0B로 진행한다.
사용자 요청 없이
...
- 설계 문서 작성 당시 별도 shell 변경을 포함한 136 tests가 통과했다. Phase -1
  구현 시점의 현재 기준선은 152 tests이며 모두 통과했다.
