...
- [ ] 확장 candidate ledger 반영 후 전체 264 unit test 재실행(2026-07-19 실행 중단, 완료 결과 없음)
...
29개를 읽어 11개 제외·18개 screening으로 남겼고, explicit multilingual probe는
50개를 읽어 22개 제외·28개 screening으로 남겼다. exact base/tree까지 실제로
materialize한 4개 중 iOS/CoreData row는 고정 Linux host에 `xcodebuild`가 없어
`resource-budget-exceeded`로 제외했다. 아래 3개는 base가 확인됐지만 나머지 inclusion
rule이 모두 통과하지 않았으므로 여전히 screening이다.
...
- 현재 확장분의 JSON Schema 검사와 focused ledger 6 tests는 통과했다.
- local 36, SWE-bench Multilingual 300, Korean snapshot 411, explicit multilingual
  ...
  검증을 수행했다.
- 현재 코드 기준 전체 suite는 264 tests가 예상되지만 2026-07-19 실행은 완료 전에
  중단됐다. 따라서 **264 tests 통과로 기록하지 않는다.**

다음 세션의 순서는 고정한다.

1. `git status --short --branch`로 이 handoff commit과 사용자 소유의 별도 untracked
   파일을 구분한다.
2. 아래 focused test를 다시 실행한다.
...
3. localhost socket을 쓰는 기존 `test_web_ui` 3개가 허용되는 환경에서 아래 전체
   suite를 실행하고 264개 완료 결과를 먼저 남긴다.
...
4. `/tmp` source artifact가 사라졌다면 ledger의 고정 query·cutoff·dataset revision으로
   ...
   provenance로 간주하지 않는다.
5. source 변경을 더 하기 전에 위 검증이 통과해야 한다. 그 다음 base를 이미 확인한
   3개, 나머지 Korean reviewed screening 15개, explicit-language reviewed screening
   28개 순서로 license, linked PR, exact base/tree, objective evaluator 가능성,
   격리·예산을 검사한다.
6. 11개 inclusion rule이 전부 `pass`가 되기 전에는 어떤 row도 `selected`로 바꾸지
   않는다. 부족 quota를 번역이나 임의 분류로 채우지 않는다.
