...
설계 검토를 위한 Claude 실행 2건은 별도 수동 review attempt이며 이 숫자에 포함하지
않는다. 다음 cohort를 분리한다.
...
- DR는 propensity support 외에도 별도의 충분한 objective-quality label과 검증된
  outcome model을 요구한다.
...
**Phase -1로 추가 오염을 먼저 멈추고, Phase 0/1의 최소 관측 기반 뒤 곧바로
