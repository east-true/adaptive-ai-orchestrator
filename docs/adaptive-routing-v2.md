...
`paired-smoke-manifest-v2`는 각 evaluator assertion의 requirement를 실제
...
task별 allowlist를 사전 등록했고 negative control, plan/validate, agent-free dry-run과
실제 paired 실행을 마쳤다. 8개 attempt 중 7개 objective quality가 관측됐고 Claude CLI
session-quota failure 한 행은 missing quality로 보존했다. partial log의 정확한 prefix와
untouched workspace를 검증한 뒤 남은 attempt만 재개했으며, 이 smoke는 pipeline
evidence일 뿐 agent ranking이나 promotion evidence가 아니다.
...
검증할 수 없다. 기존 전체 test verification은 objective quality로 재사용하지 않으므로
새 task-specific evidence가 반드시 필요하다.

그 schema와 evaluator validity review 뒤 paired pilot과 pilot 기반 확증 benchmark로
비교 가능성을 만든다. 현재 트래픽에서는 0.05 prospective overlap도 실효 표본을
만들지 못하므로 보류한다.
