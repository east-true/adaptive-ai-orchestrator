...
> 상태: v1/v2 실제 paired smoke와 v2 partial-log pause/resume audit 완료
> manifest schema: 역사적 `paired-smoke-manifest-v1`, 완료된 contract audit용 `paired-smoke-manifest-v2`
...
첫 smoke의 v1 manifest는 사후 변경하지 않고 계속 재생할 수 있다. v2 manifest는
...
agent-free dry-run과 plan/dry-run path 일치를 통과했다. 실제 실행은 attempt 8개를 모두
materialize했고 7개 objective pass와 session-quota infrastructure failure 1개를 남겼다.
중단 전 3개 attempt를 보존한 `resume`은 나머지 5개만 실행했다. 상세 결과는
`experiments/results/phase2a-smoke-v2.md`에 있다.
