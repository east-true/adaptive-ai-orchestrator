...
```text
review attempt: network-blocked
status: failed
...
```text
review attempt: source-review-completed
...
당시 실행 schema에는 stable `execution_id`가 없었다. 공개 문서에는 로컬 session UUID 대신
의미 기반 review label만 남기며, 두 시도는 `.orchestrator/executions.jsonl`의 local record로
추가됐다.
...
```text
review attempt: detailed-follow-up-completed
model: claude-opus-4-8
status: completed
duration: 851.4s
turns: 36
...
- prospective randomized cohort에서는 propensity를 설계상 정확히 기록한다.
  DR의 outcome model은 이론적 식별의 추가 필수조건이라기보다, 이 프로젝트가
  분산 감소와 robustness를 주장하기 전에 검증하도록 둔 운영 허용 조건이다.
...
## 7. 2026-07-18 연구 근거 중심 재검토와 합의

사용자 요청으로 기존 두 검토보다 범위를 넓혀 연구 근거, 통계 계약, 현재 source를
다시 읽는 비판 검토를 오케스트레이터에서 수행했다. 첫 격리 호출은 네트워크 제한으로
실패했고, 연결 허용 뒤 같은 요청을 다시 실행했다.

```text
failed review: network-blocked
terminal reason: API ENOTIMP
cost: $0

successful review: source-aware-review-completed
model: claude-opus-4-8
duration: 521.1s
turns: 34
cost: $2.5698165
...
Claude 세션에는 file read/find만 허용됐고 web과 Bash는 거부됐다. 따라서 source의
구조적 결함은 직접 확인했지만, 최신 2026 논문 원문과 440KB telemetry의 정확한
집계는 독립 확인하지 못했다. 주 검토에서 별도로 1차 자료를 열어 확인한 결과,
LLMRouterBench는 Findings of ACL 2026 논문이며 문서의 391,645 instance와 simple
baseline/model-recall 결론이 원문과 일치했다. Agent-as-a-Router, SWE-Router,
multilingual evaluator preprint의 핵심 설명도 확인했다. 최신 multilingual 자료는
계속 보조 근거로만 둔다.

재검토의 결론은 substantive redesign 없이 Phase -1을 시작할 수 있다는 것이었다.
현재 코드의 score range, token redaction, duration 분모, cost 비대칭, escalation,
exact-tier history, interrupt durability 진단도 다시 확인됐다. 제안된 수정 중
LLMRouterBench를 미확인 preprint처럼 낮추는 안은 1차 자료와 기존 문서에 맞지 않아
수용하지 않았다.

### 7.1 주 검토자의 교차판단에 대한 Claude 재검토

주 검토자는 DR의 이론/운영 조건, overall metric, target workload intake, 비용
문구와 연구 출판 상태를 교차판단해 Claude에게 다시 전달했다.
...
turns: 22
cost: $1.727474
item verdicts: agree 7, partially agree 2, disagree 0
file changes: none
```

최종 합의:

- ESTR 구조, phase 순서, invariants, paired protocol과 future OPE gate를 유지한다.
- prospective randomized cohort의 propensity는 알려진 설계값이다. overlap이 있으면
  IPS 식별이 가능하며, 검증된 outcome model은 DR의 이론적 필수조건이 아니라 분산
  감소와 추가 robustness를 주장하기 위한 프로젝트 admission rule이다.
- target workload weight가 없으면 pooled quota 평균을 workload policy value라고
  부르지 않고, agent-independent intake를 Phase 0부터 병렬 측정한다.
- 비용 결측 경고는 현재 문구가 맞다. Claude-only penalty 비대칭과 일반 CLI/plan의
  `cost_limit_usd` 입력 경로 부재는 main design에서 추가로 명시한다.
- exact field 이름은 구현 세부지만 stable execution/attempt identity와
  pair/propensity linkage의 존재는 필수 data contract다.
- LLMRouterBench는 peer-reviewed 근거로 유지하고, 2026-07 multilingual preprint의
  잠정적 보조 근거 경고도 유지한다.

최종 readiness 판정은 **Phase -1 즉시 착수 가능**이다. 위 문서 정밀화는 설계의
방향이나 phase를 바꾸지 않으며 Phase -1과 병렬로 반영할 수 있다.
