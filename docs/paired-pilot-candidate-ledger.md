...
| GitHub Korean-bearing fixed-query snapshot | 411 | screening 365, excluded 43, selected-for-task-authoring 3 | query별 최신 100개 cap, 현재 31개 manual language/category review와 contextual batch 포함 |
| GitHub explicit multilingual fixed-query snapshot | 383 | screening 326, excluded 53, selected-for-task-authoring 4 | 전체 query unique 387개 중 기존 pool과 겹친 4개를 중복 제거, 50개 deterministic review와 contextual batch 포함 |
...
| 상태 | 개수 | 의미 |
|---|---:|---|
| `screening` | 1,025 | native 언어, evaluator 가능성, license, instruction parity, 격리·예산 중 하나 이상을 아직 독립 판정하지 않음 |
| `excluded` | 98 | local 2개, Korean-bearing pool 43개, explicit multilingual pool 53개 |
| `selected-for-task-authoring` | 7 | 12개 inclusion rule을 모두 `pass`한 construction queue 후보; final pilot task가 아님 |
...
`exact_base_resolvable`이 `pass`인 행은 **66개**다. 그중 35개는
...
독립 판정이 필요한 것은 **외부 후보 31개**다. 아래는 그 31개다.
...
| `ghmix-genonai--doc_parser-issue-288` | `screening` | 11/12 | issue-derived offline evaluator의 base-negative/solution-positive와 resource 재현만 미완료 |
...
있는 27개만 판정해 pass 13 / fail 14이고, 나머지는 `unknown`이다. `Docker-Compose #38`도
...
`selected-for-task-authoring`은 현재 **7건**이다. 이는 candidate construction queue이지
...
근거는 [18차 handoff](adaptive-routing-progress.md#2026-07-22-18차-handoff--skala-66-atomic-commit-분리와-license-제외)에
고정했다.
