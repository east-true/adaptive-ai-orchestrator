...
| GitHub Korean-bearing fixed-query snapshot | 411 | screening 399, excluded 12 | query별 최신 100개 cap, 29개만 초기 수동 review/linkage probe 완료 |
...
| 상태 | 개수 | 의미 |
|---|---:|---|
| `screening` | 1,094 | native 언어, evaluator 가능성, license, 격리·예산 중 하나 이상을 아직 독립 판정하지 않음 |
| `excluded` | 36 | local 2개, Korean-bearing probe 12개, explicit multilingual probe 22개 |
...
제외했다. 총 12개는 private·빈 명세, 여러 작업 결합, live external API, manual-only UI
verification, subjective-only evaluation, resource 사유로 제외했다. screening 17개도
`selected`가 아니며 대부분 license, base tree materialization, evaluator, 격리·예산은
`unknown`이다.
