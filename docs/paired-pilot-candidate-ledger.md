...
| GitHub Korean-bearing fixed-query snapshot | 411 | screening 378, excluded 33 | query별 최신 100개 cap, 29개만 초기 수동 review/linkage probe 완료 |
| GitHub explicit multilingual fixed-query snapshot | 383 | screening 341, excluded 39, selected 3 | 전체 query unique 387개 중 기존 pool과 겹친 4개를 중복 제거, 50개 수동 review |
...
| 상태 | 개수 | 의미 |
|---|---:|---|
| `screening` | 1,053 | native 언어, evaluator 가능성, license, 격리·예산 중 하나 이상을 아직 독립 판정하지 않음 |
| `excluded` | 74 | local 2개, Korean-bearing pool 33개, explicit multilingual pool 39개 |
| `selected-for-task-authoring` | 3 | 11개 inclusion rule을 모두 `pass`한 후보 |
...
`exact_base_resolvable`이 `pass`인 행은 **52개**다. 그중 35개는
...
독립 판정이 필요한 것은 **외부 후보 17개**다. 아래는 그 17개다.

| candidate | decision | pass | 남은 것 |
|---|---|---:|---|
| `ghmix-semantic-reasoning--factlog-issue-26` | `selected` | 11/11 | — |
| `ghmix-joshua-jingu-lee--ante-issue-2349` | `selected` | 11/11 | — |
| `ghmix-e7217--anygarden-issue-512` | `selected` | 11/11 | — |
| `ghko-SeoyunL--factlog-academic-issue-314` | `screening` | 10/11 | `reproducible_within_budget` — 재현은 완료, instruction parity만 미증명 |
| `ghmix-hskim-solv--BidMate-DocAgent-issue-1152` | `screening` | 10/11 | 동일 |
| `ghmix-YSbookcase--TimePilot-issue-62` | `screening` | 10/11 | `reproducible_within_budget` — 정답 PR에 테스트가 0개라 재구성할 gold test가 없다 |
| `ghko-minacle--swift-tui-issue-18` | `screening` | 6/11 | 상류 테스트를 evaluator로 쓸 수 없음이 실측 확인됨 |
| `ghko-SeoyunL--factlog-academic-issue-342` | `screening` | 4/11 | base·license만 확정, 나머지 미심사 |
| `ghko-ohah--zntc-issue-4564` · `#4563` · `#4553` | `screening` | 4/11 | 동일 |
| `ghmix-SeokRae--blog-issue-12` | `screening` | 4/11 | 동일 |
| `ghmix-greenheadHQ--nixos-config-issue-918` | `screening` | 4/11 | 동일 |
| `ghko-itismyfield--AgentDesk-issue-4606` | `screening` | 4/11 | 동일 |
| `ghko-Chigo55--Docker-Compose-issue-38` | `excluded` | 6/11 | `subjective-only-evaluation` |
| `ghmix-seob717--nunchi-issue-35` | `excluded` | 5/11 | `subjective-only-evaluation` |
| `ghko-ProudlyOffbeat--...-MVP-iOS-issue-93` | `excluded` | 5/11 | `resource-budget-exceeded` |

#### instruction parity가 현재 지배적 blocker다

10/11 두 행이 걸려 있는 것은 재현이 아니라 **base tree가 두 CLI에게 서로 다른 지시를
주는 문제**다. Claude Code는 `CLAUDE.md`를, Codex는 `AGENTS.md`를 읽으므로 base tree가
둘 중 하나만 갖거나 둘의 내용이 다르면 fixture 자체가 비대칭이 된다.

base tree의 agent instruction 파일은 **삭제·변경하지 않는다.** 두 CLI가 받는 effective
instruction의 동등성이 증명되기 전에는 environment parity와 `reproducible_within_budget`을
`pass`로 두지 않는다. 이 기준은 원래 `factlog #314` 한 행의 메모였으나 2026-07-20 측정에서
pool 전반의 문제로 확인돼 일반 기준이 됐다. 측정과 한계는
[parity artifact](../experiments/phase2b-agent-instruction-parity-2026-07-20.json)와
[5차 handoff](adaptive-routing-progress.md#2026-07-20-5차-handoff--연구-근거-검증-instruction-parity-재현-3건)에 있다.

Claude Code 2.1.215는 실측으로 `AGENTS.md`를 읽지 않으며 `CLAUDE.md` 안의 `@AGENTS.md`
import는 해석한다. **Codex 측은 quota 소진으로 전혀 측정되지 않았다.** 그 측정 없이는
어떤 행의 parity도 판정할 수 없고, 통과 가능 행 수를 추정하지도 않는다.

`anygarden #512`가 `selected`가 될 수 있었던 것은 이 지점을 우회했기 때문이다 — 941개
...
아니다.** 남은 것은 `AGENTS.md` 1,047B와 `@AGENTS.md` import인 11바이트 `CLAUDE.md`의
parity뿐이다. 이 행의 license 근거는 원래 classifier 신호였으나 pinned revision artifact로
정정했다.
...
evaluator 0.6초 `small`이다. 남은 것은 parity뿐인데 base tree가 `CLAUDE.md` 12,250B에
`AGENTS.md`가 없어 지금까지 관측한 것 중 가장 큰 일방 비대칭이다. 별도로 기록해 둔 두
관찰: real-repo sentinel이 base에서 `AttributeError`로 먼저 죽어 문서가 실제로 깨져
있었음을 독립 입증하지는 못하며, 병합 변경이 fragment 검증 추가와 문서 4건 수정을 함께
담고 있어 `single_bounded_task`는 task authoring의 판단으로 남는다.
...
현재 합계 57 Korean/301 English/22 mixed이고 selected quota는 3/0/0이다.
...
회귀 테스트를 더해 265 tests가 됐다. 2026-07-20 기준은 **270 tests**이며 focused ledger
tests는 **12개**가 통과한다.
...
3. Codex instruction discovery 측정(2026-07-25 이후) → parity 기준 확정 → 그에 걸려 있던
   행들의 `reproducible_within_budget` 판정.
4. explicit-language reviewed 나머지 심사.
5. classifier 기반 license 근거가 남은 행 점검. `factlog #314`에서 실제로 발견돼
   정정했으므로 같은 시기 행에 남아 있을 수 있다.

`selected`는 현재 **3건**이다. quota가 부족하면 번역이나 임의 분류로 채우지 않고 부족분을
...
근거는 [5차 handoff](adaptive-routing-progress.md#2026-07-20-5차-handoff--연구-근거-검증-instruction-parity-재현-3건)에
고정했다.
