...
| GitHub Korean-bearing fixed-query snapshot | 411 | screening 400, excluded 11 | query별 최신 100개 cap, 29개만 초기 수동 review/linkage probe 완료 |
...
| 상태 | 개수 | 의미 |
|---|---:|---|
| `screening` | 1,095 | native 언어, evaluator 가능성, license, 격리·예산 중 하나 이상을 아직 독립 판정하지 않음 |
| `excluded` | 35 | local 2개, Korean-bearing probe 11개, explicit multilingual probe 22개 |
...
commit/tree를 실제 upstream에서 materialize했다. 그중 iOS/CoreData 1개는 고정 Linux
환경에 `xcodebuild`가 없어 resource 사유로 추가 제외했다. 총 11개는 private·빈 명세,
여러 작업 결합, live external API, manual-only UI verification, resource 사유로
제외했다. screening 18개도 `selected`가 아니며 대부분 license, base tree materialization,
evaluator, 격리·예산은 `unknown`이다.
...
재개 직후의 첫 gate는 확장 ledger 기준 전체 264 unit test다. 2026-07-19 실행은 완료
전에 중단되어 통과로 기록하지 않았으며, source screening을 더 바꾸기 전에 다시
실행한다. 그 다음 수동 review에서 남은 Korean 18개와 explicit-language 28개부터
