...
| GitHub Korean-bearing fixed-query snapshot | 411 | screening 354, excluded 53, selected-for-task-authoring 4 | query별 최신 100개 cap, 현재 33개 manual language/category review와 contextual/rank-7 batch 포함 |
| GitHub explicit multilingual fixed-query snapshot | 383 | screening 307, excluded 67, selected-for-task-authoring 9 | 전체 query unique 387개 중 기존 pool과 겹친 4개를 중복 제거, 50개 deterministic review와 후속 contextual/rank-7 batch 포함 |
...
| 상태 | 개수 | 의미 |
|---|---:|---|
| `screening` | 995 | native 언어, evaluator 가능성, license, instruction parity, 격리·예산 중 하나 이상을 아직 독립 판정하지 않음 |
| `excluded` | 122 | local 2개, Korean-bearing pool 53개, explicit multilingual pool 67개 |
| `selected-for-task-authoring` | 13 | 12개 inclusion rule을 모두 `pass`한 construction queue 후보; final pilot task가 아님 |
...
`exact_base_resolvable`이 `pass`인 행은 **96개**다. 그중 35개는
...
외부 후보는 **61개**다. 아래 표는 cost cascade 전에 수동 판정한 초기 외부 31개다.
...
추가 판정해 pass 9 / fail 16이므로 누적은 **52개, pass 22 / fail 30**이고 나머지는
`unknown`이다. 새 blob/path/effective-chain 근거는
...
제외하지 않은 채 뒤로 미룬다. SWE-bench Multilingual 300건은 base revision은 이미 있지만
dataset card의 MIT만으로 upstream code의 use basis가 되지 않으므로 41개 upstream
repository별 pinned license와 dataset terms를 별도 확인한다.
...
6. 다음은 기존 solution-scope review 18건 뒤에 rank 5의 MIT-at-base 반환 1건을 붙인
   19건이다. 저비용 source 의미 gate를 먼저 적용하며 no-test-touch 57건은 보류한다.
7. SWE-bench 300건은 41개 repository 단위로 upstream pinned license와 dataset terms를
   확인한 뒤 통과 행만 English task screening으로 넘긴다.
8. signal-positive queue가 소진된 뒤에만 `none-observed` 행을 exact-revision 근거로
   재검토한다. classifier나 default branch 신호만으로 pass/fail 처리하지 않는다.
9. classifier 기반 license 근거가 남은 과거 판정도 계속 점검한다. `factlog #314`에서
...
때문에 current-HEAD signal을 terminal로 승격하지 않는다. 현재 license terminal exclusion은
총 **25건**이다.
...
`selected-for-task-authoring`은 현재 **13건**이다. 이는 candidate construction queue이지
...
근거는 [23차 handoff](adaptive-routing-progress.md#2026-07-24-23차-handoff--rank-7-의미-필터와-agent-free-reproduction-완료)에
고정했다. Cascade, exact-revision, rank-6 parity, rank-7 semantics/reproduction과 세대별
ledger-application invariants를 포함한 focused suite는 37 tests, 전체 suite는 290 tests가
통과했다.
