...
`ExecutionHistory.metrics_for` now aggregates `average_cost_usd` from each logged execution's `metadata.cost_usd` when present (today, only Claude Code populates it — see `ClaudeCodeAgent.parse_result`). `AdaptiveRouter.select` applies a fixed penalty to a candidate only when the task sets `cost_limit_usd` *and* that candidate's historical average exceeds it; with no stated budget or no cost samples, cost has no effect on scoring — there's nothing to compare against, so it stays neutral rather than guessed at. `Task.cost_limit_usd` is accepted by the Python API and structured plan JSON, including generated plans; the ordinary one-task `run` CLI still has no direct budget flag.

The interactive shell's `usage` command deliberately reports two structurally different sources. Codex supplies rate-limit percentages through its own undocumented local session-log format, which may change on a Codex CLI upgrade; Claude Code supplies only its subscription tier locally, so the command pairs that account-wide label with cost samples from this project's `ExecutionHistory` and explicitly says that no live quota percentage is available. This asymmetry matches the existing `ExecutionMetadata.cost_usd` behavior where Codex exposes no cost field. A structured plan can now state `cost_limit_usd`, but the resulting historical penalty remains asymmetric while Codex cost is unknown.

## Paired smoke planning and secondary reporting

`paired plan` is a pure projection over a validated manifest object and a lexical absolute workspace root. It publishes deterministic assignments and the eight planned paths under one explicit `workspaces` field, marks both agent execution and workspace creation false, and performs no workspace-root lookup or mutation. `paired dry-run` later checks that the independently prepared exact-base checkouts use those same paths.

Paired analysis keeps every preregistered pair in the evaluator-coverage denominator. It reports lifecycle reliability, attempt and fully observed pair coverage, separately observed agent/evaluator/runner time, per-agent cost and token missingness, and modified-file observation. New paired runs protect resource metadata and modified paths in each finalized event. Compatibility replays do not infer fields absent from older events, and unexpected-file counts remain non-estimable until a manifest pins task-specific allowlists.

An infrastructure/evaluator pause finalizes the affected attempt before stopping. `paired resume` accepts only a deterministic finalized selection prefix, revalidates the existing checkout topology and the untouched suffix's exact base/fixtures, skips materialized attempt IDs, and carries prior active elapsed time into the remaining wall-time budget. It does not erase the failure row or restart the experiment under duplicate IDs.

The historical paired manifest v1 remains replayable. Manifest v2 adds a preflight audit contract: every evaluator has a non-empty assertion inventory whose requirement maps to literal text in a named task field, plus an explicit reviewer attestation that the inventory is exhaustive. The validator can prove the declared text mapping and bind it to the protected evaluator artifact hash; it deliberately does not infer assertions from evaluator source. V2 also requires normalized repository-relative exact modified-file allowlists. Glob semantics are excluded, and a total unexpected-file count is emitted only when both all task scopes and all attempt-level modified-file observations are present.
